...
public class ClassStudy : MonoBehaviour // MonoBehaviour(부모 클래스)를 상속받은 ClassStudy(하위 클래스)
{
    ...
    void Start()
    {
        /*      클레스 예시  
         *      // 클래스의 인스턴싱; 객체화
                Person p1 = new Person("민형기", 27);
                //p1.name = "민형기"; // 필드에 값을 대입
                //p1.age = 27;
        ...
        // 실습2. 주차장 관리시스템
        // 1. Car클래스 -> 차량번호, 입차시간을 필드로 가지는 클래스
        // 2. ParkingLot클래스 -> Car클래스를 넣을 수 있는 컬렉션으로 자동차를 관리(List, Queue, Stack)
        ...
        // 순서2. 등록 버튼 클릭 시 클릭 이벤트 메서드 실행 -> ParkingLot 클래스의 차량 입차 메서드 실행 -> input의 내용 기반 차량 등록
        // 순서3. 등록 버튼 클릭 시 현재 주차된 차량 목록 출력(업데이트)
        // 순서4. 출차 버튼 클릭 시 현재 주차된 차량 목록에서 해당 내용 제거

        // 차량 입차 메서드: 5138 입력 후 등록버튼 클릭 시 실행, 차량번호, 입차시간을 가지고 있는 Car 클래스 new 키워드로 만든다.
    
    }

    public void OnRegisterBtnCarClkEvent()
    {
        if(parkingLot == null)
        {
    ...
    public void OnOutBtnCarClkEvent()
    {
